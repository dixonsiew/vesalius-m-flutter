...
class BP extends StatefulWidget {
  ...
  static const String routeName = '/VitalSigns/BP';
...
class _BPState extends State<BP> {
  ...
  void load() async {
    try {
      ...
      List<VitalSignsData> lx = await getVitalSignHistory('BP', branchDetails!.branch!.branchId!, branchDetails.prn!, widget.date);
      setState(() {
        list = lx;
        isLoading = false;
      });
  ...
  Widget buildChart() {
    return Container(
      ...
      height: MediaQuery.of(context).orientation == Orientation.portrait ? MediaQuery.of(context).size.height / 2.5 : MediaQuery.of(context).size.height - 90.0,
      decoration: const BoxDecoration(
        color: Colors.white,
        border: Border(
          bottom: BorderSide(
    ...
    return Padding(
      ...
      child: Row(
        ...
        children: [
          ...
          Text(
            '${data.value1}/${data.value2} ${data.unit}',
            style: const TextStyle(
              fontSize: 18.0,
              fontFamily: kBodyFont,
              color: Color(0xFF727272),
