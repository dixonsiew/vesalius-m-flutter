...
import 'package:vesalius_m_flutter/controllers/services/golden-pearl/confirm_join_club_activity_ctrl.dart';
...
class _ConfirmJoinClubActivityState extends State<ConfirmJoinClubActivity> {
  ...
  final GoldenPearlCtrl goldenPearlCtrl = Get.put(GoldenPearlCtrl());
            ...
            RichText(
              text: TextSpan(
                children: [
                  ...
                  TextSpan(
                    text: '${joinClubActivityCtrl.goldenPearlActivity?.goldenActivityName ?? ''} ',
                    style: kTextStyle1.copyWith(
                      fontSize: 14.0,
                      fontWeight: FontWeight.w600,
                      color: kTextColor1,
  ...
  Future<void> refreshMainList() async {
    try {
      List<GoldenPearlActivity> lx = await ClubsService.getAllGoldenPearlActivities(1, 2, 1);
      goldenPearlCtrl.setList(lx);
    }
  ...
  void onSubmit() async {
    try {
      final a = joinClubActivityCtrl.goldenPearlActivity;
      final la = selectMemberCtrl.selectedMemberList.map((x) {
        return {
          'goldenActivityId': a?.goldenActivityId,
          'goldenMembershipId': x.goldenMembershipId,
          'activityDateTime': formatDate(ctrl.date!, [dd, '/', mm, '/', yyyy])
        };
      }).toList();
      final o = {
        'goldenActvParticipation': la
      };
      ctrl.setIsLoading(true);
      if (AuthManager.instance.isLogin) {
        await ClubsService.postGoldenPearlActivityJoin(o);
      }

      else {
        await GuestModeService.postGoldenPearlActivityJoin(o);
      }
  ...
  List<Widget> buildList() {
    ...
    lx.addAll([
      ...
      Container(
        ...
        child: Text(
          joinClubActivityCtrl.goldenPearlActivity?.goldenActivityName ?? '',
          style: kTextStyle1.copyWith(
            fontSize: 16.0,
            fontWeight: FontWeight.w400,
            color: kTextColor1,
    ...
    lx.addAll([
      ...
      Container(
        ...
        child: Material(
          ...
          child: InkWell(
            onTap: () async {
              final lx = joinClubActivityCtrl.goldenPearlActivity?.datesMinMax ?? [DateTime.now(), DateTime.now()];
