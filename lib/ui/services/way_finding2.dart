...
import 'way-finding/map_view_route.dart';
import 'way-finding/qrscan.dart';
import 'way-finding/search_location.dart';
import 'way-finding/underpass_view.dart';
...
class _WayFinding2State extends State<WayFinding2> {
  ...
  ScrollController scrFloor = ScrollController();
  ...
  void load() async {
    try {
      ...
      SchedulerBinding.instance.addPostFrameCallback((_) {
         scrFloor.jumpTo(scrFloor.position.maxScrollExtent);
      });
  ...
  void loadRoute() async {
    try {
      ...
      if (ctrl.route == null) {
        ...
        Get.to(() => MapViewRoute(
          data: ctrl.route!,
          from: ctrl.selectedLocationFrom!,
          to: ctrl.selectedLocationTo!,
        ));
      }
    }
  ...
  Color getSelectedFloorBgColor(WayFinding o) {
    ...
    if (ctrl.route != null) {
      ...
      // if (ctrl.selectedLocationFrom?.locationFloorCode == o.floorCode) {
      //   c = const Color(0xFF00ACFC);
      // }

      // if (ctrl.selectedLocationTo?.locationFloorCode == o.floorCode) {
      //   c = const Color(0xFFE1001A);
      // }
    }
  ...
  List<Widget> buildList() {
    ...
    for (int i = 0; i < ctrl.list.length; i++) {
      ...
      final m = SizedBox(width: 40.0, child: k);
      lx.add(m);
  ...
  String getImageTitle() {
    String s = 'Macalister Wing';
    if (ctrl.selectedFloor.contains('PW')) {
      s = 'Peel Wing';
    }
        ...
        children: [
          ...
          Row(
            children: [
              ...
              InkWell(
                onTap: () {
                  Get.to(() => UnderpassView(data: 'Underpass To Macalister Wing'));
                },
                child: Text(
                  'Macalister Wing',
                  style: kTextStyle1.copyWith(
              ...
              InkWell(
                onTap: () {
                  Get.to(() => UnderpassView(data: 'Underpass To Peel Wing'));
                },
                child: Text(
                  'Peel Wing',
                  style: kTextStyle1.copyWith(
          ...
          if (ctrl.selectedLocationFrom?.locationBuilding != ctrl.selectedLocationTo?.locationBuilding) ...[
            SizedBox(height: 8.0),
            Text(
              "If you are from PEEL WING, you may use the LIFT LOBBY B (E4,5,6) at G floor at the lobby to direct to the basement (B)",
              style: kTextStyle1.copyWith(
                fontSize: 14.0,
                fontWeight: FontWeight.w400,
                color: kPrimaryColor,
            ...
            SizedBox(height: 8.0),
            Text(
              "If you are from MACALISTER WING, you may use the LIFT (E6) at G floor near the Accident & Emergency (A&E) to direct to the basement (B)",
              style: kTextStyle1.copyWith(
                fontSize: 14.0,
                fontWeight: FontWeight.w400,
                color: kPrimaryColor,
