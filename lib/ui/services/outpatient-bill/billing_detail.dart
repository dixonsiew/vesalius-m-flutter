...
import 'package:vesalius_m_flutter/controllers/services/outpatient-bill/billing_detail_ctrl.dart';
...
class _BillingDetailState extends State<BillingDetail> {
  ...
  void onSubmitIpay() async {
    try {
      ctrl.setIsLoading(true);
      final patientData = await DataManager.instance.getPatientDetails();
      final data = widget.data;
      String docNum = '';
      if (patientData != null) {
        Document? doc = patientData.documents.firstWhereOrNull((o) => o.code == 'ID');
        if (doc != null) {
          docNum = doc.value ?? '';
        }
      }
      final o = {
        'userBilling': {
          'patientPrn': patientData?.prn ?? '',
          'patientName': patientData?.displayName ?? '',
          'patientDocNumber': docNum,
          'vesRegDateTime': data.regDateTime,
          'vesBillNumber': data.billNumber,
          'vesInvoiceNumber': data.invoiceNumber,
          'vesInvoiceDateTime': data.billInvoiceDateTime,
          'vesInvoiceAmount': data.invoiceAmount,
          'vesOutstandingAmount': data.outstandingAmount
        },
        'userBillingPayment': {
          'billingFullname': txtfullname.text,
          'billingAddress1': txtaddr1.text,
          'billingAddress2': txtaddr2.text,
          'billingAddress3': txtaddr3.text,
          'billingTowncity': txtcity.text,
          'billingState': txtstate.text,
          'billingPostcode': txtpostcode.text,
          'billingCountryCode': ctrl.selectedCountry?.countryCode ?? '',
          'billingContactNo': txtcontact2.text,
          'billingContactCode': txtcontact1.text,
          'billingEmail': txtemail.text
        },
      };
      final res = await UserBillingService.postPurchaseIpay(o);
      ctrl.setIsLoading(false);
  ...
  void onSubmit() async {
    try {
      if (ctrl.selectedCountry?.countryCode != 'ID') {
        onSubmitIpay();
        return;
      }
      ...
      final wallexRes = await UserBillingService.postPurchaseWallex(o);
      ctrl.setIsLoading(false);
      if (wallexRes != null) {
        Get.offUntil(GetPageRoute(page: () => CheckoutOK(data: wallexRes)), (route) => (route as GetPageRoute).routeName == OutpatientBill.routeName);
      }
    }
