...
import 'package:vesalius_m_flutter/controllers/services/medical-history/vital-signs/bmi_ctrl.dart';
...
class _BMIState extends State<BMI> {
  ...
  final BMICtrl ctrl = Get.put(BMICtrl());
  ...
  void load() async {
    try {
      ...
      List<VitalSignsData> lx = await VesaliusService.getVitalSignHistory('BMI', branchDetails!.branch!.branchId!, branchDetails.prn!, widget.date);
      ctrl.setList(lx);
  ...
  List<Widget> buildListItem() {
    ...
    for (int i = 0; i < ctrl.list.length; i++) {
      ...
      lx.add(HistoryItem(
        ...
        val: o.value1 == null ? '' : '${o.value1} kg/m\u00B2',
      ));
