...
class VesaliusService {
  ...
  static Future<List<Allergy>> getPatientAllergies(num branchId, String prn) async {
    ...
    try {
      final res = await ApiHelper.tokenDioInterceptor.get('$kServerUrl/$name/patient-allergy/$prn');
  ...
  static Future<List<PatientVisit>> getVesaliusPatientVisit(num branchId, String prn, num pageId) async {
    ...
    try {
      final res = await ApiHelper.tokenDioInterceptor.get('$kServerUrl/$name/patient-visit/$prn/$pageId'); // 20015952
      if (res.statusCode == 204) {
        lx = [];
        return lx;
      }
  ...
  static Future<List<VitalSignsData>> getVitalSignHistory(String type, num branchId, String prn, String dates) async {
    ...
    try {
      final res = await ApiHelper.tokenDioInterceptor.get('$kServerUrl/$name/get-vital-signs-history/$prn/$dates/$type');
      if (res.statusCode == 204) {
        lx = [];
        return lx;
      }
  ...
  static Future<List<VitalSignsHistory>> getVitalSignHistories(num branchId, String prn) async {
    ...
    try {
      final res = await ApiHelper.tokenDioInterceptor.get('$kServerUrl/$name/get-vital-signs-history/$prn');
      if (res.data == null) {
        lx = [];
        return lx;
      }
  ...
  static Future<List<LabHistory>> getLabHistories(num branchId, String prn) async {
    ...
    try {
      final res = await ApiHelper.tokenDioInterceptor.get('$kServerUrl/$name/get-lab-history/$prn');
      if (res.data == null) {
        lx = [];
        return lx;
      }
  ...
  static Future<File> getHealthScrReportPdf(num branchId, String refno, String fp, void Function(int, int) onReceiveProgress) async {
    try {
      final res = await ApiHelper.tokenDioInterceptor.get('$kServerUrl/$name/health-screening-report/$refno',
